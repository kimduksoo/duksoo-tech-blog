...
마지막이 가장 위험했다. 화이트리스트에 등록된 명령으로 시작하면, `;`으로 체이닝해서 아무 명령이나 실행할 수 있었다. 명령 체이닝 차단(`;` `&&` `||`), 절대경로 우회 차단, 추가 위험 명령(`tee`, `xargs`, `env`) 패턴을 반영해서 수정했다.

### Sandboxed Evaluation: 격리된 환경에서 검증

프로덕션 채널에 바로 연결하지 않았다. 테스트 전용 Slack 채널을 만들고, 에이전트를 이 <span style="color:#1565c0; font-weight:bold">격리된 평가 환경(Isolated Evaluation Environment)</span>에서 2주간 운영했다.

사례 1의 비용 분석 에이전트 반복 테스트, 사례 2의 파라미터 스토어 에이전트 32회 반복도 모두 이 환경에서 수행했다. 프로덕션 데이터에 영향을 주지 않으면서 실제와 동일한 조건으로 검증할 수 있었다.

모든 에이전트 실행은 JSON 트랜스크립트로 기록된다. 에이전트가 어떤 도구를 호출했고, 어떤 판단을 내렸고, 최종 출력이 무엇이었는지 전수 감사(Audit)가 가능하다. 트랜스크립트를 리뷰하면서 의도하지 않은 동작을 잡아내고, 프롬프트를 수정하는 사이클을 반복했다.

### 트리아지: 에이전트 호출 자체를 필터링

에이전트가 안전하게 동작하는 것만큼 중요한 게, 잘못된 메시지가 에이전트를 호출하지 않는 것이다. Slack 채널의 모든 메시지가 에이전트를 트리거하면 오작동과 비용 낭비가 발생한다.

2단계 트리아지로 이를 차단한다:

1. **키워드 필터** (결정론적): "파라미터", "비용", "배포" 등 사전 정의된 키워드로 1차 필터링. 매칭되지 않는 메시지는 즉시 무시한다.
2. **Haiku 3.5 분류** (비결정론적): 키워드에 매칭된 메시지를 경량 모델(Haiku)이 분류한다. "이 메시지가 실제로 인프라 요청인지, 단순 대화인지" 판단해서 적절한 에이전트를 결정한다.

키워드 필터는 비용이 0이고, Haiku 분류는 건당 ~$0.0002다. 비용 분석을 수행하는 Opus($0.5/건)가 불필요하게 호출되는 것을 방지한다.
