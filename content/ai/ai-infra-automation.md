...
Haiku에는 MCP 서버도, 도구도 연결하지 않았다(`mcp_servers={}`, `allowed_tools=[]`). 싱글턴(`max_turns=1`)으로 설정해서 멀티턴 대화도 불가능하다. 분류와 라우팅만 수행하고 어떤 변경도 할 수 없는 구조다. 작업의 성격에 따라 적절한 모델 티어가 다르다. 분류에는 Haiku(건당 ~$0.0002, 응답 1초 이내), 분석과 실행에는 Opus(건당 ~$0.5)를 사용한다.
