...
- **충분한 컨텍스트**: 200K 토큰 컨텍스트로, 4개 계정의 비용 데이터를 조회하고 비교 분석하고 리포트를 작성하기까지 맥락을 잃지 않는다
- **지시 준수**: 프롬프트에 분석 기준과 판단 임계값을 명시하면, 일관되게 따른다. 운영 노하우를 프롬프트로 코드화할 수 있는 기반이다
- **도구 선택**: 상황에 따라 어떤 도구를 어떤 순서로 쓸지 스스로 판단한다. Slack 읽기 → AWS 조회 → 분석 → Slack 쓰기를 하나의 흐름으로 수행한다
