...
일일 분석의 패턴을 주간 리포트에도 적용했다. Python이 4개 계정의 주간 비용을 사전 계산하고, 에이전트는 분석과 리포트만 쓴다.

첫 주간 리포트에서 RDS -92.5% 급감을 "이상 징후"로 잡고 인프라 팀을 멘션했다. 확인해보니 전전주에 RI 선결제가 있었던 것이다. 이번 주가 비정상이 아니라, 전전주가 높았던 것.
...
일일 분석에서는 "숫자가 맞는가"를 고쳤고, 주간 리포트에서는 "판단이 맞는가"를 고쳤다.

이후 만든 모든 분석 에이전트(월간, Datadog)에 동일한 패턴을 적용했다.

---

## 핵심 사례: 파라미터 스토어 등록 에이전트
...
비용 분석에서 배운 "계산은 코드가, 판단은 AI가" 원칙을 여기에도 적용했다. 하지만 이번에는 새로운 문제가 있었다. <span style="color:#1565c0; font-weight:bold">에이전트가 프로덕션 인프라를 변경한다</span>. 잘못되면 서비스에 영향을 준다.
