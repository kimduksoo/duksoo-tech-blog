...
한 건에 15~30분이지만, 매일 2~3건씩 반복되면 주 5~10시간의 TOIL이 된다. TOIL은 SRE(Site Reliability Engineering)에서 사용하는 용어로, 반복적이고 자동화 가능하며 전략적 가치 없이 시간만 소모하는 수작업을 뜻한다. SRE의 핵심 목표 중 하나가 이 TOIL을 줄이는 것이다. 그보다 더 큰 문제는, 사람은 반복에 지치면 놓친다는 것이다. 놓친 이상은 발견될 때까지 조용히 누적된다.
