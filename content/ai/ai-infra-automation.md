...
> **Human**: 그러면 어떻게 해야 하지? API 실패 시 재시도하고, 데이터가 불완전하면 리포트를 만들지 않게 할 수 있나?
>
> **AI**: 두 가지 방법이 있습니다:
> 1. **프롬프트에 검증 규칙 추가** — API 8개 전부 성공해야만 리포트 작성
> 2. **코드로 데이터 수집을 분리** — Python이 API를 호출하고, 에이전트는 분석만
>
> 1번이 간단하니 먼저 시도해보겠습니다.

### 1차 수정: 프롬프트에 검증 규칙 추가

데이터 검증 규칙을 프롬프트에 추가했다.
...
3번 시도하고 3번 실패했다. API 호출과 산술 연산은 같은 입력이면 항상 같은 결과가 나와야 하는 결정론적(Deterministic) 작업이다. 비결정론적(Non-deterministic)인 LLM은 분석과 판단에는 강하지만, 이런 결정론적 작업에서는 한계가 있었다. 에이전트가 제시한 대안 2번, 계산 로직을 코드로 분리하는 방식을 선택했다.
...
일일 분석이 안정되고 나니, 주간 리포트도 같은 방식으로 만들면 되겠다 싶었다. 4개 AWS 계정의 비용을 합산하고, 전주 대비 변동을 분석하는 주간 리포트다.

첫 시도에서는 에이전트에게 API 호출까지 맡겼다.

> **프롬프트 (v1)**: CloudCheckr OAuth2 인증 후, 4개 계정의 이번 주/전주 비용을 조회하고 분석해서 Slack에 리포트를 올려라
>
> **AI**: OAuth2 토큰 발급 → 8번 API 호출(4계정 × 2기간) → 토큰 만료 → 재발급 → 재시도...
>
> 5분이 지나서야 리포트가 올라왔다.

일일 분석에서 겪은 문제가 그대로 반복됐다. 같은 실수를 반복하지 않기로 하고, "계산은 코드가" 패턴을 바로 적용했다.

> **프롬프트 (v2)**: 아래 사전 계산된 데이터를 기반으로 Slack 리포트를 작성하라
> ```
> 총 비용:
> - 전주: $3,572.93
> - 전전주: $6,857.68
...
>
> 서비스별:
> RDS    $240.69  (전전주 $3,216.18, -92.5%)
> EC2    $955.22  (전전주 $1,264.73, -24.5%)
> ```
>
> **AI** *(21초 후)*:
> ```
> AWS 주간 비용 리포트 ⚠️
> 전주 총 비용: $3,572.93
> 전전주 대비: -$3,284.75 (-47.9%) ← 주의 필요
>
> 이상 징후:
> • RDS -92.5% 급감 ($3,216 → $241)
> @infra 확인 부탁드립니다.
> ```

5분에서 21초. 일일 분석의 교훈이 바로 적용됐다.

그런데 리포트를 보니, RDS가 -92.5% 급감한 걸 "이상 징후"로 잡고 인프라 팀을 멘션했다. 확인해보니 전전주에 RDS Reserved Instance 선결제가 있었던 것이다. 이번 주가 비정상이 아니라, 전전주가 비정상적으로 높았던 것.

비용이 줄어든 건 좋은 소식인데, 매주 담당자를 호출하면 알림 피로가 생긴다.

> **Human**: 비용 감소는 좋은 거잖아. 원인이 확인된 감소에 @멘션 걸지 마
>
> **AI**: 멘션 규칙을 수정하겠습니다:
> - 비용 **증가** + 원인 미확인 → @infra 멘션
> - 비용 **감소** → 멘션 없이 보고만
> - RI 선결제 같은 **확정 원인** → (확정) 표시, 멘션 불필요

일일 분석에서는 "숫자가 맞는가"를 고쳤고, 주간 리포트에서는 "판단이 맞는가"를 고쳤다. 단계마다 다른 문제가 나왔지만, 매번 사람이 확인하고 수정하는 과정이 시스템을 단단하게 만들었다.

이후 만든 모든 분석 에이전트(월간, Datadog)에 동일한 패턴을 적용했다.
