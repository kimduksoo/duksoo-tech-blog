...
1. **Sandbox** — 격리된 평가 환경(Isolated Evaluation Environment). 테스트 전용 채널에서 개발자 1명이 반복 검증. 사례 1의 비용 분석 반복 테스트, 사례 2의 파라미터 스토어 32회 반복이 모두 이 단계에서 수행됐다.
2. **Canary** — 소수 인원이 참여하는 내부 채널로 이동. 실제 사용자의 자연어 입력과 예상치 못한 요청 패턴을 확인했다.
3. **Production** — 검증이 완료된 에이전트만 프로덕션 타겟 채널에 연결했다.
