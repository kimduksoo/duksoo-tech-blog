...
- **비용**: 입력 토큰 기준 $5.00/M으로, GPT-5.2($1.75/M), Gemini 3 Pro($2.00/M) 대비 2.5~3배 비싸다. 실행당 약 $0.5, 하루 5~10건이면 월 $75~$150
- **속도**: 프론티어 모델 중에서도 느린 편이다. 같은 Claude 내에서도 Sonnet 4.5 대비 체감 3~10배 느리고, 코딩 작업 실측에서 Gemini 3 Pro, GPT-5.2보다 뒤처진다. 단, 같은 작업에 19~76% 적은 토큰을 사용해 토큰 효율은 높다
- **벤더 락인**: LLM마다 잘하는 분야가 다르다. 코딩은 Claude가, 추론은 Gemini·GPT가, 속도는 Gemini Flash가 앞선다. 하지만 Claude Agent SDK 기반으로 구성했기 때문에, 작업별로 최적의 모델을 선택하는 유연성이 없다
