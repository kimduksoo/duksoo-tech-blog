...
### 설계: Claude Code와 아키텍처를 잡다

위 다이어그램의 구조는 처음부터 있었던 게 아니다. 에이전트라는 개념 자체가 익숙하지 않은 상태에서, Claude Code에 "에이전트를 만든다는 게 뭔지 설명해줘"라고 물어보는 것부터 시작했다. 개념 설명을 듣고 hello_agent를 만들어 실행해본 뒤, 본격적으로 구조를 잡기 시작했다.
...
> | 레벨 | 내용 |
> |------|------|
> | 1. YAML | `allowed_tools`로 도구 제한 |
> | 2. 코드 | `safe_bash.py`로 명령어 패턴 검사 |
> | 3. 프롬프트 | 에이전트 지시사항에 "AWS는 읽기만" 명시 |
...
### 프로덕션 첫날: AI가 이상 징후를 먼저 잡다

아키텍처를 변경하고 프로덕션에 올린 첫 주, 에이전트가 주간 비용 리포트를 자동 생성하면서 이상 징후를 먼저 잡았다. 주간 비용이 전주 대비 -47.9% 급감한 것을 탐지한 것이다. 프롬프트에 정의된 임계값(±15%)을 초과하자, 에이전트가 자동으로 경고 리포트를 작성하고 담당 팀에 에스컬레이션했다.

에이전트가 Slack에 올린 리포트 일부:

> *이상 징후* @infra
> - 프로덕션 계정 -51.2% 급감
> - EC2 -91.7% 급감 — 가장 큰 비용 감소 요인
> - ELB -72.8% 감소
>
> *권장 조치*
...
사람이 매일 리포트를 열어보지 않아도, 이상이 있으면 AI가 먼저 잡아서 알려준다.
...
시스템 프롬프트는 3,328자에서 10,076자로 3배 늘었다. 늘어난 건 기능이 아니라 제약 조건이다. <span style="color:#1565c0; font-weight:bold">최소 권한 원칙(Principle of Least Privilege)</span>을 프롬프트에 적용한 것이다. 할 수 있는 것을 나열하는 대신, 하면 안 되는 것을 명시했다. 에이전트의 자유도를 제한할수록 안전해진다.
...
### Sandboxed Evaluation: 격리된 환경에서 검증

프로덕션 채널에 바로 연결하지 않았다. 3단계 <span style="color:#1565c0; font-weight:bold">Staged Rollout(단계적 배포)</span>으로 Blast Radius(장애 영향 범위)를 점진적으로 넓혔다.
...
주간 배포 리포트 에이전트가 스케줄에 따라 자동 실행되면서, 비정상 배포 패턴을 잡아냈다. 특정 서비스(stt-service)가 하루에 317회 반복 배포된 것을 탐지한 것이다. 해당일 전체 481회 배포 중 65.9%가 이 서비스 하나였다.

에이전트는 프롬프트에 정의된 이상 기준을 자동 적용하고, 원인 가설과 함께 @infra 팀에 에스컬레이션했다.

> *이상 징후* @infra
> - 01-14(화) stt-service 배포 317회 급증 — 비정상 패턴
>
> *분석 의견*
> - 배포 스크립트 또는 ArgoCD 설정 점검 권장

에이전트가 사람 대신 매주 배포 현황을 감시하고, 비정상 패턴을 자동으로 잡아낸다.

### 트리아지: 에이전트 호출 자체를 필터링
...
2. **Haiku 3.5 분류** (비결정론적): 키워드에 매칭된 메시지를 경량 모델(Haiku)이 분류한다. "이 메시지가 실제로 인프라 요청인지, 단순 대화인지" 판단해서 적절한 에이전트를 결정하고, 에이전트 호출에 필요한 사전 컨텍스트를 준비한다.
...
에이전트가 읽기 전용 분석을 넘어 변경 작업(Parameter Store 등록, K8s 서비스 생성 등)을 수행하려면, 사람의 명시적 승인이 필요하다. <span style="color:#1565c0; font-weight:bold">에이전트가 수립한 실행 계획을 사람이 검토하고, 승인해야만 실제 변경이 이루어진다.</span> 에이전트의 판단이 틀렸거나 의도와 다른 계획을 세웠을 때, 실행 전에 차단할 수 있는 마지막 방어선이다.
...
1. **Tool Use (도구 호출)**: 에이전트가 외부 도구를 직접 호출할 수 있어야 한다. Slack 메시지를 읽고, AWS CLI로 데이터를 조회하고, 다시 Slack에 결과를 쓰는 과정을 하나의 흐름에서 수행해야 한다.
2. **MCP 지원 (외부 서비스 네이티브 연동)**: Slack, Jira, Datadog 같은 외부 서비스를 API 래핑 없이 네이티브로 연결할 수 있어야 한다. 서비스가 추가될 때마다 연동 코드를 작성하면 유지보수 부담이 커진다.
3. **긴 컨텍스트 (맥락 유지)**: 비용 데이터 조회 → 계정별 비교 → 이상 징후 탐지 → 리포트 작성까지, 멀티스텝 분석에서 앞선 맥락을 잃지 않아야 한다.

이 기능 요구사항을 기준으로 모델과 프레임워크를 선택했다. Agent SDK + MCP 조합은 세 가지를 모두 충족한다. 에이전트가 상황에 따라 어떤 도구를 쓸지 스스로 결정하고, MCP로 연결된 외부 서비스를 네이티브로 호출한다. n8n이나 LangChain 같은 워크플로우 도구도 고려했지만, 사전에 흐름을 정의하는 방식은 "판단이 필요한 업무"에 맞지 않았다.

Claude Code는 이 시스템 자체를 만드는 데 사용했다. 아키텍처를 논의하고, 코드를 만들고, 테스트하고, 실패하면 같이 원인을 찾고 고치는 반복을 대화형으로 진행했다. 위의 프롬프트 로그가 그 과정이다.
...
- **도구 사용**: MCP를 통해 Slack, Jira, Datadog을, SafeBash를 통해 AWS CLI를 직접 사용한다. API 래핑 코드가 필요 없다
...
- **비용**: Opus 기준 실행당 약 $0.5. 하루 5-10건이면 월 $75-$150
