...
이 기능 요구사항을 기준으로 모델과 프레임워크를 선택했다. Agent SDK + MCP 조합은 세 가지를 모두 충족한다. 에이전트가 상황에 따라 어떤 도구를 쓸지 스스로 결정하고, MCP로 연결된 외부 서비스를 네이티브로 호출한다. n8n도 고려했지만, AI와 대화하면서 만들어가는 방식에 맞지 않았다. 초기에는 Cursor에서 여러 모델을 써봤는데, 개인적으로 Opus 4.5의 신뢰도가 가장 높았다. 이 모델을 더 자유롭게 쓸 수 있는 Claude Code 환경으로 옮겼다. Claude Agent SDK는 이 로컬 업무 환경을 에이전트로 그대로 이식할 수 있고, API를 별도 구매하지 않아도 계정 사용량 안에서 바로 써볼 수 있어서 선택했다.
