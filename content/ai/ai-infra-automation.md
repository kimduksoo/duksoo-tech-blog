...
이 글에서 다룬 건 비용 분석과 파라미터 등록, 두 가지 사례다. 하지만 같은 구조로 확장한 에이전트가 더 있다. Datadog 사용량 분석, 배포 현황 리포트, 모니터링 알림 분석까지—현재 7개 이상의 에이전트가 프로덕션에서 운영 중이다.

핵심 과제는 반복 업무를 자동화하면서 LLM의 불확실성을 통제하는 것이었다. 할루시네이션은 결정론적 작업(API 호출, 계산)을 코드로 분리해서 해결했고, 무분별한 실행은 트리아지와 승인 게이트로 막았다. 안전장치는 AI로 AI를 공격하는 Red Teaming으로 검증했다.

주 5~10시간이던 반복 업무가 자동으로 처리되고, 사람은 승인과 예외 케이스에만 개입한다. AI 에이전트는 만능이 아니다. 하지만 제약을 명확히 설계하면, 프로덕션에서도 신뢰할 수 있는 자동화 도구가 된다.
