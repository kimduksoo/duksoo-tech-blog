...
Agent에는 다음 도구를 제공했다:
- **Slack MCP**: 리포트 메시지 읽기, 분석 결과 댓글 작성
- **AWS CLI**: Cost Explorer, CloudWatch 등으로 상세 데이터 조회

핵심은 **이상 판단 기준**이다. 단순히 "비용이 올랐다"가 아니라, 맥락을 고려해야 한다.

| 판단 기준 | 예시 |
|----------|------|
| 전일 대비 25% 이상 변동 | EC2 비용이 어제보다 30% 증가 |
| 신규 비용 항목 출현 | 기존에 없던 Bedrock 비용 발생 |
| 특정 계정 급증 | dev 계정 비용이 prod보다 높아짐 |

이상이 감지되면 Agent가 `@infra`를 태그해서 담당자에게 확인을 요청한다. 이상이 없으면 리포트만 남기고 넘어간다.
...
일일 리포트가 안정화되자 주간, 월간 리포트도 추가했다. 일일 리포트가 "오늘 뭐가 변했나"라면, 주간 리포트는 "이번 주 트렌드가 어떤가"를 본다.
...
일일 리포트에서는 놓치기 쉬운 **점진적 증가 패턴**을 주간 리포트에서 잡을 수 있다. 하루 단위로는 정상 범위인데, 주 단위로 보면 계속 올라가고 있는 경우다.

### 실제 감지 사례: CloudFront 비용 $1,170/월 절감

주간 리포트에서 Agent가 CloudFront 비용 이상을 감지하고 `@infra`로 확인을 요청했다. 담당자가 조사해보니, 프론트엔드에서 무한 스크롤 시 이미지를 원본 크기로 요청하고 있었다. 37개 카드 이미지(총 7.5MB)가 매 요청마다 캐시 없이 전송되고 있었던 것이다.

이미지 최적화와 캐시 설정을 적용해서 월 $1,170을 절감했다. 상세 내용은 [프론트엔드 최적화로 CloudFront 비용 월 $1,170 절감한 이야기](/finops/cloudfront-cost-frontend-optimization/)에서 다룬다.

이 시스템이 없었다면 월말 리뷰에서야 발견했을 것이다. 주간 리포트 덕분에 조기에 잡았다.
