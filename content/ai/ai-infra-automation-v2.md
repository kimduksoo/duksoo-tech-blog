...
- 특정 채널에서만 동작하기 때문에 **어디에 써야 하는지** 알아야 한다
- 키워드가 없으면 필터에서 무시되기 때문에 **어떻게 써야 하는지** 알아야 한다
- 에이전트가 작업 중인지, 승인 대기인지, 완료인지 Slack 스레드를 직접 따라가야 **뭘 하고 있는지** 알 수 있다
...
추가된 것이 하나 있다. 요청이 들어오면 에이전트가 즉시 Jira 티켓을 생성한다. 기존에는 승인 후 수작업으로 만들었지만, 지금은 요청 접수 시점에 자동 생성되고, 작업 완료 후 결과가 댓글로 달린다.
