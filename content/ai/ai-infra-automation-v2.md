...
다른 팀원이 인프라 요청을 하려면 특정 채널에서 특정 키워드를 포함해서 글을 써야 했다. "NAT IP 알려주세요"는 반응하지만 "외부에서 접근하려는데 IP가 뭐야?"는 키워드 필터에서 무시된다.

키워드를 모르면 에이전트가 없는 것과 같다.

### 유지보수하는 사람 입장

새 채널이나 에이전트를 추가하려면 3개 파일을 건드려야 했다.

| 파일 | 수정 내용 |
|------|----------|
| `event_triggers.yaml` | 트리거 설정 + 키워드 목록 |
| `slack_listener.py` | 라우팅 분기 추가 |
| `triage.py` | Haiku 프롬프트 수정 |

키워드 목록은 점점 길어지고, 엣지케이스마다 예외를 추가해야 했다. "파라미터"는 매칭되는데 "파라메터"는 안 되고, "배포"는 되는데 "디플로이"는 안 되는 식이다. 동작하지만 유지보수가 부담스러운 구조였다.
...
1편이 "AI 에이전트가 일을 할 수 있는가?"에 대한 답이었다면, 이번 글은 "다른 사람도 쓸 수 있는가?"에 대한 답이다.

복잡한 라우팅을 걷어내니 채널 추가가 쉬워졌고, 키워드를 몰라도 에이전트가 반응하게 됐다. 기술적으로 어려운 일은 아니었다. 채널 매핑은 dictionary lookup이다. 어려운 것은 <span style="color:#1565c0; font-weight:bold">"어디를 단순화해야 사용자가 편해지는가"</span>를 찾는 일이었다.

하지만 채널 라우팅만으로는 부족했다. 에이전트가 지금 뭘 하고 있는지, 어떤 요청이 처리되고 있는지 Slack 스레드를 직접 따라가야 하는 문제가 남아있었다. [다음 편](/ai/ai-infra-automation-v3/)에서는 이 문제를 해결한 웹 콘솔 이야기를 한다.
