...
```mermaid
flowchart LR
    M_WAL[WAL<br/>Prometheus TSDB 포맷] --> M_Mem[메모리<br/>시계열 샘플] --> M_Block[TSDB 블록<br/>2시간마다 컴팩션] --> M_S3[(S3)]
```
...
```mermaid
flowchart LR
    L_WAL[WAL<br/>세그먼트 + 체크포인트] --> L_Mem[메모리<br/>로그 청크] --> L_S3[(S3)]
```
...
```mermaid
flowchart LR
    T_Mem[메모리<br/>Live Trace] --> T_WAL[WAL<br/>Parquet 포맷] --> T_Block[Completed Block] --> T_S3[(S3)]
```
