...
3가지 모두 "Pod 단위로 최소 권한을 부여한다"는 원칙을 충족하기 어렵다.

IRSA는 AWS가 EKS에 네이티브로 제공하는 솔루션으로, 추가 컴포넌트 없이 Pod 단위 IAM 권한 분리가 가능하다.
...
### 같은 namespace에서 SA별로 다른 권한이 가능하다

하나의 namespace에 여러 ServiceAccount를 만들고, 각각 다른 IAM Role을 연결할 수 있다. 예를 들어 같은 namespace의 API 서버는 S3 읽기만, Batch 서버는 S3 읽기+쓰기 권한을 부여할 수 있다.
