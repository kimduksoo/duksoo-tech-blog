...
아직 적용하지 않은 개선 사항들이 있다.

| 항목 | 내용 | 상태 |
|------|------|------|
| 민감 서비스 PDB + TopologySpread 추가 | do-not-disrupt와 이중 안전망 구성 | 검토 중 |
| r7i NodePool RI 활용도 개선 | 민감 서비스를 r7i 노드에 고정하여 RI 낭비 방지 | 검토 중 |
| 인스턴스 사이즈 업 검토 | t3.xlarge(4 vCPU) → t3.2xlarge(8 vCPU) | RI 만료 시점에 검토 예정 |
