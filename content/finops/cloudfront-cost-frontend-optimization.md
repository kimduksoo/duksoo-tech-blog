...
CDN 도메인 `cdn.ajd.kr`이 전체 증가분의 대부분을 차지했다. 요청 수는 11.2%만 늘었는데, 평균 응답 크기가 205KB에서 278KB로 35.6% 커졌다. **같은 수의 요청을 처리하는데 더 많은 데이터를 보내고 있었다.**
...
배포 이력을 확인했다. 문제 시점에 `aj-platform-front` 프로젝트에서 카드 목록/상세 페이지가 신규 배포되었다.

```
1/21 배포: PLAT-582 카드 리스트 및 상세페이지 퍼블리싱 및 API 연동
          - 67개 파일 변경
...
카드 페이지의 프론트엔드 코드를 확인했다. 세 가지 문제가 있었다.
...
### 문제 2: Infinite Scroll / Pagination 미적용

카드 목록을 한 번에 전체 로딩하고 있었다. 페이지네이션이나 무한 스크롤 없이 전체 카드 데이터를 한 번의 API 호출로 가져왔다.
...
### 2. Infinite Scroll 적용

한 번에 전체 목록을 로딩하는 대신, 페이지당 20개씩 로딩하도록 변경했다.

```javascript
const { data, infinityData } = await useInfinityScroll(
  'card-list',
  () => useFetchCardList({
...
사용자가 스크롤하면 다음 20개를 추가 로딩한다. 초기 로딩 시 전송되는 이미지가 37개에서 최대 20개로 줄었고, 실제로 사용자가 스크롤하지 않으면 첫 화면에 보이는 이미지만 로딩된다.
...
```mermaid
flowchart LR
    Before["변경 전<br/>카드 목록 접속 시<br/>37개 × 300~540KB<br/>= 7.5MB/요청"] --> After["변경 후<br/>뷰포트 내 이미지만<br/>~5개 × 150~270KB<br/>= ~1MB/요청"]
...
| 항목 | 변경 전 | 변경 후 | 절감 |
|------|---------|---------|------|
| 주간 CloudFront 비용 | $1,238 | ~$1,000 | ~$238/주 |
| 월간 환산 | ~$5,360 | ~$4,330 | ~$1,030/월 |

Lazy loading과 infinite scroll만으로도 카드 페이지의 전송량이 크게 줄었고, 이미지 포맷 변환까지 적용하면 전체 CDN 전송량 자체가 줄어드는 효과가 있었다.
